...
*)
*/
let BO={device:`gpu`,publicPath:`${self.location.origin}/models/`,debug:!0,model:`isnet`,progress:(e,t,n)=>{let r=Math.round(t/n*100);self.postMessage({type:`progress`,payload:{key:e,current:t,total:n,percent:r}}),console.log(`Progress [${e}]: ${r}% (${t}/${n})`)},output:{format:`image/png`,quality:1}};self.onmessage=async e=>{let{type:t,payload:n}=e.data;if(t===`preload`)try{await RO(BO),self.postMessage({type:`preload-complete`})}catch(e){self.postMessage({type:`error`,payload:{message:e instanceof Error?e.message:`Preload failed`}})}else if(t===`remove-bg`)try{let{file:e}=n,t=await zO(e,BO),r=e instanceof File?`${e.name.replace(/\.[^/.]+$/,``)}.png`:`output.png`,i=await t.arrayBuffer();self.postMessage({type:`complete`,payload:{data:i,mimeType:t.type,fileName:r}})}catch(e){self.postMessage({type:`error`,payload:{message:e instanceof Error?e.message:`Unknown error`}})}}})();