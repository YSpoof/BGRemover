...
*)
*/
let BO=self.location.origin,VO=`preload`,HO=null,UO={device:`gpu`,publicPath:`${BO}/models/`,debug:!0,model:`isnet`,progress:(e,t,n)=>{let r=Math.round(t/n*100);self.postMessage({type:`progress`,payload:{phase:VO,itemId:HO,key:e,current:t,total:n,percent:r}}),console.log(HO?`Progress [${HO}] [${e}]: ${r}% (${t}/${n})`:`Progress [${e}]: ${r}% (${t}/${n})`)},output:{format:`image/png`,quality:1}};self.onmessage=async e=>{let t=e.data;if(t.type===`preload`){VO=`preload`,HO=null;try{await RO(UO),self.postMessage({type:`preload-complete`})}catch(e){self.postMessage({type:`error`,payload:{phase:`preload`,itemId:null,message:e instanceof Error?e.message:`Preload failed`}})}}else if(t.type===`remove-bg`){let{file:e,itemId:n}=t.payload;try{VO=`remove`,HO=n;let t=await zO(e,UO),r=e instanceof File?`${e.name.replace(/\.[^/.]+$/,``)}.png`:`output.png`,i=await t.arrayBuffer();self.postMessage({type:`complete`,payload:{itemId:n,data:i,mimeType:t.type,fileName:r}})}catch(e){self.postMessage({type:`error`,payload:{phase:`remove`,itemId:n,message:e instanceof Error?e.message:`Unknown error`}})}finally{HO=null}}}})();